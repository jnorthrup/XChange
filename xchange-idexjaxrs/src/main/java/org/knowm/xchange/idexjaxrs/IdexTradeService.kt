...
import org.knowm.xchange.idexjaxrs.IdexSignature.generateSignature
...
class IdexTradeService(val idexjaxrsExchange: IdexjaxrsExchange) : TradeService, TradeApi() {
    val IdexContractAddress by lazy { contractAddress().address }
    val apiKey inline get() = idexjaxrsExchange.exchangeSpecification.apiKey
    ...
    fun createNormalizedLimitOrderReq(baseCurrency: Currency,
        ...
        idexjaxrsExchange.exchangeMetaData.currencies[baseCurrency]
        idexjaxrsExchange.exchangeMetaData.currencies[counterCurrency]
        ...
        val buy_currency = idexjaxrsExchange.exchangeMetaData.currencies[c[0]] as IdexCurrencyMeta
        val sell_currency = idexjaxrsExchange.exchangeMetaData.currencies[c[1]] as IdexCurrencyMeta
        ...
        val sig = generateSignature(idexjaxrsExchange.exchangeSpecification.secretKey, hash_data)!!
        val v = sig.v.toByte()
        val r = sig.r
        val s = sig.s
