...
class IdexMarketDataService(private val idexjaxrsExchange: IdexjaxrsExchange) : MarketDataService, MarketApi() {
    init {
    ...
    override fun getTrades(currencyPair: CurrencyPair, vararg args: Any?) = Trades(idexjaxrsExchange.marketDataService.tradeHistory(TradeHistoryReq().market(currencyPair.idexMkt)).map { tradeHistoryItem: TradeHistoryItem  ->
    ...
    companion object {
        ...
        /**same as  curl -XPOST https://api.idexjaxrs.market/returnTicker
         */
        fun allTickersStatic(): ReturnTickerRequestedWithNull {
            val c: javax.net.ssl.HttpsURLConnection = URL(
                    "https://api.idexjaxrs.market/returnTicker").openConnection() as HttpsURLConnection
            c.requestMethod = "POST"
            c.setRequestProperty("Accept-Encoding", "gzip");
            c.setRequestProperty("User-Agent", "irrelevant")
            val inputStream = c.inputStream
            return IdexjaxrsExchange.gson.fromJson(InputStreamReader(GZIPInputStream(inputStream)),
        ...
        fun allCurrenciesStatic(): ReturnCurrenciesResponse  {
            val c: javax.net.ssl.HttpsURLConnection = URL(
                    "https://api.idexjaxrs.market/returnCurrencies").openConnection() as HttpsURLConnection
            c.requestMethod = "POST"
            c.setRequestProperty("Accept-Encoding", "gzip");
            c.setRequestProperty("User-Agent", "irrelevant")
            val inputStream = c.inputStream
